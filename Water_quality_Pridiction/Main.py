...
if not os.path.exists(MODEL_FILE):
    ...
    labels=[1, 2, 3, 4, 5]
     ...
     # Fill NaNs with mode
    mode_val = df["ph_cat"].mode()[0]
    df["ph_cat"] = df["ph_cat"].fillna(mode_val)
    ...
    if not os.path.exists('input.csv'):
        if os.path.exists('water_quality_dataset_100k_new.csv'):
            df = pd.read_csv("water_quality_dataset_100k_new.csv")
            df["ph_cat"] = pd.cut(
                    df["pH"],
                    bins=[0, 1.5, 3.0, 4.5, 6.0, np.inf],
            ...
            split = StratifiedShuffleSplit(n_splits=1, test_size=0.2, random_state=42)
            for train_index, test_index in split.split(df, df['ph_cat']):
                df.loc[test_index].drop("ph_cat", axis=1).to_csv("input.csv", index=False)
            print("'input.csv' file automatically generated")
        else:
            print("Error: 'water_quality_dataset_100k_new.csv' file nahi mili, isliye 'input.csv' generate nahi ho sakti.")
            exit(1)
